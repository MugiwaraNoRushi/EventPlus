...
class FrameAxis:
	def __init__(self, mfd=None, w2v_model=None):
			...
			if mfd == "emfd":
					emfd = pd.read_csv(
					f'{current_dir_path}/../../component/Embedding_MF/eMFD_wordlist.csv')
					self.axes, mfs = self._get_emfd_axes(emfd)
			elif mfd == "mfd":
					mfd = pd.read_csv(
					f'{current_dir_path}/../../component/Embedding_MF/MFD_original.csv')
					self.axes, mfs = self._get_axes(mfd)
			elif mfd == "customized":
				custom = pd.read_csv(f'{current_dir_path}/../../component/Embedding_MF/customized.csv')
