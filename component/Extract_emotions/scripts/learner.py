...
class EvaluateOnTest(object):
    ...
    """
    def __init__(self, model, test_data_loader, model_path,file):
        self.model = model
    ...
    def predict(self, device='cuda:0', pbar=None):
        ...
        :returns: None
        """
        output_file = self.file#'data/test/output.txt'
        ...
        pd.DataFrame(data=preds_dict['y_pred'],columns=['pred_'+str(i) for i in range(len(preds_dict['y_pred'][0]))]).to_csv(output_file,index=False)
        str_stats = []
